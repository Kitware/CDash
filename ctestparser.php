...
{   
  ...
  $start_time = gmdate(FMT_DATETIME,$starttimestamp);
  $end_time = gmdate(FMT_DATETIME,$endtimestamp);
...
function parse_update($parser,$projectid)
{
  ...
  // Find the build id
  $buildid = get_build_id($buildname,$stamp,$projectid,$sitename);
  if($buildid<0)
    {
...
function parse_note($parser,$projectid)
{
  ...
  for ($i = 0; $i < sizeof($notes); $i++)
    ...
    if($parser->vals[$index]["type"] == "close")
      ...
      $time = gmdate(FMT_DATETIME,$timestamp);  
