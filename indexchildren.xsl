...
<xsl:template name="builds">
  ...
  <xsl:if test="count($type/build)=0">
    <tr class="table-heading1 table-nobuild">
      ...
      <!-- quick links -->
      <td align="right" class="nob" colspan="15">
      <div>
  ...
  <xsl:if test="count($type/build)>0">
   ...
   <tr valign="middle">
      ...
      <!-- display the label(s) as a link to the build summary -->
      <td align="left" class="paddt">
      <xsl:if test="count(labels/label)=0">(none)</xsl:if>
      <xsl:if test="count(labels/label)=1">
        <a><xsl:attribute name="href">buildSummary.php?buildid=<xsl:value-of select="buildid"/></xsl:attribute><xsl:value-of select="labels/label"/></a>
      </xsl:if>
...
<div id="site" align="left">
  <b>Site</b>: <a><xsl:attribute name="href">viewSite.php?siteid=<xsl:value-of select="/cdash/siteid"/>&#38;project=<xsl:value-of select="/cdash/dashboard/projectid"/>&#38;currenttime=<xsl:value-of select="/cdash/dashboard/unixtimestamp"/></xsl:attribute><xsl:value-of select="/cdash/site"/></a>
...
<div id="buildname" align="left">
  <b>Build Name</b>:
  <xsl:value-of select="/cdash/buildname"/>
  <xsl:if test="string-length(/cdash/buildplatform)>0">
    <img class="icon" alt="platform">
        <xsl:attribute name="src">images/platform_<xsl:value-of select="/cdash/buildplatform"/>.png</xsl:attribute>
    </img>
  </xsl:if>
</div>
<br/>
...
<xsl:for-each select="cdash/buildgroup">
  ...
  <xsl:attribute name="class">tabb childbuild <xsl:value-of select="sortlist"/></xsl:attribute>
...
<!-- Dynamic analysis -->
<table border="0" cellpadding="4" cellspacing="0" width="100%" class="tabb child" id="dynamicanalysistable">
