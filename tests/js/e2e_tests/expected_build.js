...
describe("expected_build", function() {
  it("toggle expected", function() {
    ...
    var folderIcon = element(by.repeater('build in buildgroup.builds').row(0)).all(by.tagName('img')).get(1);
    ...
    browser.get('index.php?project=InsightExample');
    element(by.repeater('build in buildgroup.builds').row(0)).all(by.tagName('img')).get(1).click();
