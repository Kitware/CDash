//
// This calendar has been modified heavily for integration into Dart.
// For a general javascript calendar, please see the original version
// of this calendar by Robert Husted.
//
...
calDateFormat = "Weekday, Month dd, yyyy";  // Default format used by Dart
calDateFormat = "yyyyMMDD";  // Dart's BuildStamp format 
...
blankCellColor   = "lightgrey";
noDataCellColor  = "lightgrey";
futureCellColor  = "lightgrey";
todayCellColor   = "green";
focusCellColor   = "black";
...
function setDateField(dateField) {
    ...
    // Original URL, stored so we can build a url to redirect to
    originalURL = self.location.href;
}
...
function setInitialDate() {
    ...
    // What is today?  Colored differently than other days
    todayDate = new Date();
    
    // What day did we start the calendar at? Colored differently 
...
function buildBottomCalFrame() {       
 ...
 var paddingChar = "&nbsp;";
        ...
        // Do we have any data for the day being written in the calendar?
        // This formats the date as a Dart datestamp.  Would like to call
        // formatDate() but that routine access some global variables
        var cDate = new Date( year, month, currentDay );
        var noDataForDay = false;
        var today = false;
        cDateStamp = "YYYYMMDD";
        cDateStamp = jsReplace(cDateStamp, "YYYY", cDate.getFullYear());
        cDateStamp = jsReplace(cDateStamp, "MM", 
                               makeTwoDigit(cDate.getMonth()+1));
        cDateStamp = jsReplace(cDateStamp,"DD",makeTwoDigit(currentDay));
        if (typeof(dashboardStartTimes[cDateStamp]) == "undefined") {
           noDataForDay = true;
        }

        // Is this day in the future?
        var futureDay = false;
        if (cDate.getTime() > todayDate.getTime())
            futureDay = true;

        // SET THE TYPE OF DAY, THE focusDay and currentDay GENERALLY 
        ...
        if ( currentDay == todayDay && month == todayMonth && year == todayYear) {
            ...
            today = true;
            dayType = "today";
            cColor = todayCellColor;
        } 
        else if (futureDay) {
            // currentDay is in the future
            dayType = "future";
            cColor = futureCellColor;
        }
        else if (currentDay == startDay && month == startMonth && year == startYear ) {
            // currentDay is the day we are already on
            dayType = "focusDay";
            cColor = focusCellColor;
        } 
        else if (noDataForDay) {
            // no data for currentDay
            dayType = "noData";
            cColor = noDataCellColor;
        }
        else {
            // a day with data
            dayType = "weekDay";
        ...
        if (!noDataForDay || futureDay) {
           calDoc += "<TD align=center bgcolor='" + cColor + "'>" + padding +
               "<a class='" + dayType + "' href='javascript:parent.opener.returnDate(" + 
               currentDay + ");'>"  + currentDay + "</a>" + paddingChar + "</TD>";
        }
        else {
           calDoc += "<TD align=center bgcolor='" + cColor + "'>" + padding +
               "<b class='" + dayType + "'>" + currentDay + "</b>" + paddingChar + "</TD>";
        }
        columnCount++;
...
function buildCalParts() {
    ...
    blankCell = "<TD align=center bgcolor='" + blankCellColor + "'>&nbsp;&nbsp;&nbsp;</TD>";
    ...
    calendarBegin =
        ...
        "<STYLE type='text/css'>" +
        "<!--" +
        "TD.heading { text-decoration: none; color:" + headingTextColor + "; font: " + headingFontStyle + "; font-size: 12pt}" +
        "TD { font-size: 12pt; font-family : arial, helvetica, sans-serif;}" + 
        ".focusDay { text-decoration:none; font-weight: bold; color: " + focusColor + "; }" + 
        ".focusDay a { text-decoration: none; }" + 
        ".focusDay a:hover { text-decoration: none; }" + 
        ".weekDay { text-decoration: none; font-weight: bold; color: " + dateColor + "; }" + 
        ".weekDay a { text-decoration: none; }" + 
        ".weekDay a:hover { text-decoration: none; }" + 
        ".today { text-decoration: none; font-weight: bold; color: " + todayColor + "; }" + 
        ".today a { text-decoration: none; }" + 
        ".today a:hover { text-decoration: none; }" + 
        ".noData { text-decoration: none; font-weight: bold; color: " + noDataColor + "; }" + 
        ".noData a { text-decoration: none; }" + 
        ".noData a:hover { text-decoration: none; }" + 
        ".future { text-decoration: none; font-weight: bold; color: " + futureColor + "; }" + 
        ".future a { text-decoration: none; }" + 
        ".future a:hover { text-decoration: none; }" + 
        "-->" +
        "</STYLE>" +
        "</HEAD>" +
...
function returnDate(inDay)
{
    ...
    // Note that this routine accesses the global variable originalURL
    // which was set in the function setDateField().

    // format the date
    outDate = formatDate(inDay);
    ...
    // tack on the selected datestamp
    if ( dashboardStartTimes[outDate] == "None")
       url = urlPrefix + outDate + "-Nightly/Dashboard.html";
    else
       url = urlPrefix + outDate + "-" + dashboardStartTimes[outDate] + "-Nightly/Dashboard.html";
*/
    // We don't use the location.replace() method here because we want the
    // the browser history to be kept.
    // So instead of
    // 
    //     if (document.images)
