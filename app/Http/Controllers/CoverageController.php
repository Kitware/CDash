...
class CoverageController extends AbstractBuildController
{
    ...
    public function apiCompareCoverage(): JsonResponse
    {
        ...
        $build_data = self::apiCompareCoverage_get_build_data(null, $projectid, $beginning_UTCDate,
            $end_UTCDate);
        ...
        $build_data = self::apiCompareCoverage_get_build_data($aggregate_build['id'], $projectid, $beginning_UTCDate, $end_UTCDate, $filter_sql);
        ...
        foreach ($response['builds'] as $build_response) {
            ...
            $build_data = self::apiCompareCoverage_get_build_data($buildid, $projectid, $beginning_UTCDate, $end_UTCDate, $filter_sql);
            ...
            if (array_key_exists('coverages', $coverage_response)) {
                foreach ($coverage_response['coverages'] as $coverage) {
                    ...
                    foreach ($coverages as $key => $subproject_response) {
                        if ($subproject_response['label'] == $coverage['label']) {
                            $coverages[$key] = self::apiCompareCoverage_populate_subproject($coverages[$key], 'build'.$buildid, $coverage);
                            break;
                        }
                    }
                }
    ...
    private static function apiCompareCoverage_populate_subproject(array $subproject, $key, array $coverage): array
    {
    ...
    private static function apiCompareCoverage_get_build_label($buildid, array $build_array): string
    {
    ...
    private static function apiCompareCoverage_get_build_data($parentid, $projectid, $beginning_UTCDate, $end_UTCDate, string $filter_sql=''): array
    {
