...
class AdminController extends AbstractController
{
    ...
    /**
     * TODO: (williamjallen) Rewrite this to take advantage of Blade templating.
     */
    public function generateCTestConfig(): View|RedirectResponse|\Illuminate\Http\Response
    {
        @set_time_limit(0);
        ...
        usort($subprojectids, function ($a, $b) {
            $va = self::get_graph_depth($a, 0);
            $vb = self::get_graph_depth($b, 0);
            if ($va == $vb) {
                return 0;
            }
            return ($va < $vb) ? -1 : 1;
        ...
        return Response::make($ctestconfig, 200, $headers);
    }

    /**
     * TODO: (williamjallen) This recursive function makes an incredible number of database queries due to the repeated
     *       creation of SubProject objects.  This should be rewritten in a more efficient manner...
     */
    private static function get_graph_depth(int $a, int $value): int
    {
        $SubProject = new SubProject();
        $SubProject->SetId($a);
        $parents = $SubProject->GetDependencies();
        foreach ($parents as $parentid) {
            $subvalue = self::get_graph_depth($parentid, $value + 1);
            if ($subvalue > $value) {
                $value = $subvalue;
            }
        }
