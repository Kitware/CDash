...
class EmailTestCase extends KWWebTestCase
{
    ...
    public function testEmailSentToGitCommitter(): void
    {
        ...
        $expected = [
            ...
            '*Failing Tests*',
            "curl | Completed | ({$url}/tests/",
            "DashboardSendTest | Completed (OTHER_FAULT) | ({$url}/tests/",
            "StringActionsTest | Completed (OTHER_FAULT) | ({$url}/tests/",
            "MathActionsTest | Completed (OTHER_FAULT) | ({$url}/tests/",
            '-CDash on',
            'user1@kw',
            'FAILED (t=4): EmailProjectExample - Win32-MSVC2009 - Nightly',
            'A submission to CDash for the project EmailProjectExample has failing tests.',
