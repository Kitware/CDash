...
function FiltersController($scope, $rootScope, $http, $timeout) {
  ...
  $scope.addFilter = function(block, index) {
    const previous_filter = block.filters[index-1];
    let filter_to_copy;
    if (previous_filter.hasOwnProperty('filters')) {
      filter_to_copy = previous_filter.filters[previous_filter.filters.length - 1];
    }
    else {
      filter_to_copy = previous_filter;
    }
    const filter = {
  ...
  $scope.changeFilter = function(filter) {
    const type = $scope.filterdefinitions[filter.field].type;
    const comparisons = $scope.comparisons[type];
  ...
  $scope.applyFilters = function() {
    const url = this.createHyperlink();
  ...
  $scope.createHyperlink = function() {
    ...
    const n = $scope.filterdata.filters.length,
    ...
    const filterParams = ['filtercount', 'filtercombine', 'showfilters', 'field', 'compare', 'value', 'limit', 'date', 'begin', 'end'];
    ...
    if ($scope.cdash.begin && $scope.cdash.end) {
      ...
      const date = $scope.cdash.begin ? $scope.cdash.begin : $scope.cdash.end;
      s += `date=${date}&`;
    }
    else if (!$scope.cdash.begin && !$scope.cdash.end) {
    ...
    for (let i = 1; i <= n; i++) {
      if ($scope.filterdata.filters[i-1].hasOwnProperty('filters')) {
        const num_subfilters = $scope.filterdata.filters[i-1].filters.length;
        if (num_subfilters < 1) {
          continue;
        }
        ...
        for (let j = 1; j <= num_subfilters ; j++) {
  ...
  $scope.addFilterBlock = function(index) {
    const parent_filter = $scope.filterdata.filters[index-1];
    const or_block = {
      filters: [],
    };
    const filter = {
  ...
  const filename_for_docs = filename;
  ...
  $http({
    ...
    filterdata.availablenoncontentfilters.forEach((availablefilter, index, object) => {
      const filter_definition = $scope.filterdefinitions[availablefilter];
...
angular.module('CDash')
  .directive('filterButtons', (VERSION) => {
    return {
